...
这是典型的**异质容器 (Heterogeneous Container)** 实现。

- **Q: 复制构造/赋值？**
    
    - **应该实现（深拷贝）。** 因为 `_ppStu` 是指针数组，默认拷贝只是浅拷贝指针。如果原对象销毁，新对象的指针将悬空（Dangling Pointer）。
        
    - **难点：** 你只存了 `Student*`，怎么在拷贝时知道它实际是 `Graduate` 还是 `Student` 并 `new` 出正确的对象？
        
    - **解法：** 需要在 `Student` 体系中实现 `virtual Student* clone() const = 0;` (原型模式)。
...
- **Q: Add 函数实现**
    ...
    - **Source 92-104 程序段分析：**
        ...
        - `stuList.add(s)`: `s` 是 `new` 出来的，没问题，控制权移交。
            
        - `stuList.add(&g)`: **严重错误！** `g` 是栈上对象（局部变量）。`add` 函数认为它接管了指针。当 `StudentList` 析构时调用 `delete`，会试图释放栈内存，导致 Crash。且 `g` 离开作用域会自动析构，导致 Double Free。
